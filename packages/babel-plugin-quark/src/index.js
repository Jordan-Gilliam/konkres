...
const validImportSources = ['quark', 'quark/asyncComponent'];
  ...
  return {
    ...
    visitor: {
      ...
      ObjectExpression(path, { opts }) {
        ...
        // if there was not property to match conditions just return
        if (!component) return;
...
const importVisitor = {
  Import(path) {
    ...
    if (chunkNameNodeIndex !== -1) {
      ...
      loaderArguments[0].insertQuark(objectToAppend);
    }
  },
};
